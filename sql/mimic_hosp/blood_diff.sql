DROP MATERIALIZED VIEW IF EXISTS blood_diff;
CREATE MATERIALIZED VIEW blood_diff AS (
	WITH blood_diffs AS (
		SELECT MAX
			...
			MAX ( CASE WHEN itemid = 52073 THEN valuenum WHEN itemid = 51199 THEN valuenum / 1000.0 ELSE NULL END ) AS eosinophils_abs, -- 51133 in K/uL, 52769 in #/uL
			MAX ( CASE WHEN itemid = 51133 THEN valuenum WHEN itemid = 52769 THEN valuenum / 1000.0 ELSE NULL END ) AS lymphocytes_abs, -- 52074 in K/uL, 51253 in #/uL
		...
		WHERE
			le.itemid IN (
				...
				51300,
				51301,
				51755 -- 52220 (wbcp) is percentage
	...
	SELECT
		...
		ROUND((CASE WHEN basophils_abs IS NULL AND basophils IS NOT NULL AND impute_abs = 1 THEN basophils * wbc ELSE basophils_abs END)::numeric, 4) AS basophils_abs,
		ROUND((CASE WHEN eosinophils_abs IS NULL AND eosinophils IS NOT NULL AND impute_abs = 1 THEN eosinophils * wbc ELSE eosinophils_abs END)::numeric, 4) AS eosinophils_abs,
		ROUND((CASE WHEN lymphocytes_abs IS NULL AND lymphocytes IS NOT NULL AND impute_abs = 1 THEN lymphocytes * wbc ELSE lymphocytes_abs END)::numeric, 4) AS lymphocytes_abs,
		ROUND((CASE WHEN monocytes_abs IS NULL AND monocytes IS NOT NULL AND impute_abs = 1 THEN monocytes * wbc ELSE monocytes_abs END)::numeric, 4 ) AS monocytes_abs,
		ROUND((CASE WHEN neutrophils_abs IS NULL AND neutrophils IS NOT NULL AND impute_abs = 1 THEN neutrophils * wbc ELSE neutrophils_abs END)::numeric, 4) AS neutrophils_abs,
		basophils,
		eosinophils,
		lymphocytes,
		monocytes,
