...
if data_mode == 'MIMIC':
    ...
    (x_dim, num_event, event_prob), (data, time, label, diags) = utils.import_dataset_eicu('normal')
    eval_times = [20, 40, 60, 80]
elif data_mode == 'SEER':
    (x_dim, num_event, event_prob), (data, time, label, diags) = utils.import_dataset_seer('normal')
...
for out_itr in range(3):
    ...
    df.to_csv(cindex_respath + '/fg_cindex_causal_{}.csv'.format(out_itr + 1))
    print(df)
    print('mean: ')
