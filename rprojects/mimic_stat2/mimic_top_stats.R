df <- read.csv('data/icu_diag_first2.csv')
...
# 组合统计
dnames <- c('Septicemia/Sepsis', 'Neoplasm', 'Cerebral Hemorrhage', 'Acute Respiratory Failure', 
            'Myocardial Infarction', 'Heart Failure', 'Pneumonia/Pneumonitis', 'Cerebral Infarction', 
            'Subarachnoid Hemorrhage', 'Cirrhosis of Liver', 'Acute Kidney Failure')
...
library(dplyr)
df_akf <- subset.data.frame(df, diag == 1 | diag == 4 | diag == 7 | diag == 2 | diag == 6 | diag == 11)
...
df_all_rdup['total11'] <- df_all_rdup$sep + df_all_rdup$neop + df_all_rdup$ch + df_all_rdup$arf + 
