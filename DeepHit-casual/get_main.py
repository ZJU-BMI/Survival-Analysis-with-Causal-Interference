...
def f_get_minibatch(mb_size, x, label, time, diags, mask1, mask2):
    ...
    return x_mb, k_mb, t_mb, d_mb, m1_mb, m2_mb
...
def get_valid_performance(DATA, MASK, in_parser, out_itr, eval_time=None, MAX_VALUE=-99, OUT_ITERATION=5, seed=1234):
    ...
    if data_mode == 'MIMIC':
        ...
        event_prob = [0.875, 0.583, 0.708]      # EICU
    ...
    for itr in range(iteration):
        if stop_flag > 5:  # for faster early stopping
            ...
            x_mb, k_mb, t_mb, d_mb, m1_mb, m2_mb = f_get_minibatch(mb_size, tr_data, tr_label,
            ...
            if (itr + 1) % 1000 == 0:
                print('|| ITR: ' + str('%04d' % (itr + 1)) + ' | Loss: ' + colored(str('%.4f' % (avg_loss)), 'yellow',
                                                                                   attrs=['bold']))
                avg_loss = 0
            ...
            if (itr + 1) % 1000 == 0:
                ...
                for t, t_time in enumerate(eval_time):
                    ...
                    if eval_horizon >= num_Category:
                        ...
                        va_result1[:, t] = -1  # va_result2[:, t] = -1
                    else:
                        risk = np.sum(pred[:, :, :(eval_horizon + 1)], axis=2)  # risk score until eval_time
                        for k in range(num_Event):
                            # va_result1[k, t] = c_index(risk[:,k], va_time,
                            # (va_label[:,0] == k+1).astype(int), eval_horizon) #-1 for no event (not comparable)
                            va_result1[k, t] = weighted_c_index(tr_time, (tr_label[:, 0] == k + 1).astype(int),
                                                                risk[:, k], va_time,
