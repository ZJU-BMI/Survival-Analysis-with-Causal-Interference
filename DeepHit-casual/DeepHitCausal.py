...
class Model_DeepHit:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            # out = out * tf.reshape(tf.tile(tf.reshape(self.d, [-1, 1]), [1, self.num_Category]),
            #                              [-1, self.num_Event, self.num_Category])
            self.out = out
    ...
    def loss_Log_Likelihood(self):
        ...
        tmp1 = tf.reduce_sum(tf.reduce_sum(self.fc_mask1 * self.out, reduction_indices=2),
                             reduction_indices=1, keep_dims=True)
        tmp1 = I_1 * log(tmp1)
        # for censored: log \sum P(T>t|x)
        tmp2 = tf.reduce_sum(tf.reduce_sum(self.fc_mask1 * self.out, reduction_indices=2),
                             reduction_indices=1, keep_dims=True)
        tmp2 = (1. - I_1) * log(tmp2)
