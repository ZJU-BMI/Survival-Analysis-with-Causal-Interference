...
def cumulative_hazard_function(risk_score, times, labels, fail_code, n_times=100):
    ...
    for i in range(N):
        ...
        cumulative_hazard[int(times[i])] = n_fails * 1 / sum_risk_score
        n_fails = 0
...
def get_mask(data_mode, time, label, num_event):
    ...
    if os.path.isfile(mask_path):
        ...
        # time must have been sorted in ascending order
        N = len(time)
        T = np.reshape(time, [N])
        E = np.reshape(label, [N])
        mask = np.zeros([N, N, num_event])
        ...
        for i in range(N):
            if E[i] > 0:
                ev = E[i] - 1
                w_ij = np.zeros(shape=[N, ])
                idx = np.where(T >= T[i])
                w_ij[idx] = 1
                mask[i, :, ev] = w_ij

        np.save(mask_path, mask)
        print('mask construction complete, save mask to local disk...')
    print('load mask complete...')
    return mask


def import_dataset_mimic(norm_mode='normal', loadmask=True):
    data_path = 'datasets/mimic_data_final2.csv'
