...
if data_mode == "EICU":
    ...
    df = pd.read_csv('./datasets/synthetic_comprisk.csv')
    df = df.sample(random_state=seed, frac=0.1)
    # print(df.label.value_counts())
    x = np.asarray(df.iloc[:, 4:12])
    x = get_normalization(x)
    y = np.asarray(df['label'])
    t = np.asarray(df['time'])
    diags = np.ones([x.shape[0], 2], dtype=np.float32)
    event_prob = [1., 1.]
else:
