DROP MATERIALIZED VIEW IF EXISTS icu_diag_first CASCADE;
CREATE MATERIALIZED VIEW icu_diag_first AS(
WITH subjects AS(
	SELECT *
		-- 1 septicemia/sepsis
...
-- 2 neoplasm
, neop AS (
		SELECT subject_id, hadm_id, 1 AS neop
		FROM subj_diag
		WHERE lower(long_title) like '%neoplasm%'
		GROUP BY subject_id, hadm_id
)
...
SELECT s.subject_id, s.hadm_id, s.hospital_expire_flag, s.diag, s.icd_code, s.long_title
							, (CASE WHEN sep.sep IS NULL THEN 0 ELSE 1 END) as sep
 							, (CASE WHEN neop.neop IS NULL THEN 0 ELSE 1 END) as neop
							...
							, (CASE WHEN diab.diab IS NULL THEN 0 ELSE 1 END) as diab
 							, (COALESCE(sep.sep, 0) + COALESCE(neop.neop, 0) + COALESCE(ch.ch, 0) + 
				...
				LEFT JOIN sep ON s.subject_id = sep.subject_id AND s.hadm_id = sep.hadm_id
 				LEFT JOIN neop ON neop.subject_id = s.subject_id AND neop.hadm_id = s.hadm_id
...
-- WHERE lower(long_title) LIKE '%kidney disease%' GROUP BY icd_code, icd_version, long_title ORDER BY long_title 
