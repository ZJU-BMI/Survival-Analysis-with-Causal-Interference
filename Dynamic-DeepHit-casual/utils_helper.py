...
def f_get_minibatch(mb_size, x, x_mi, label, time, diags, mask1, mask2, mask3):
    ...
    d_mb = diags[idx, :].astype(float)  # events diagnosis with the subject, e.g. [1, 0, 1, 1, 0]
    m1_mb = mask1[idx, :, :].astype(float)  # fc_mask
    m2_mb = mask2[idx, :, :].astype(float)  # fc_mask
    m3_mb = mask3[idx, :].astype(float)  # fc_mask
    return x_mb, x_mi_mb, k_mb, t_mb, d_mb, m1_mb, m2_mb, m3_mb


def f_get_boosted_trainset(x, x_mi, time, label, diags, mask1, mask2, mask3):
    ...
    new_diags = np.zeros([total_sample, np.shape(diags)[1]])
    ...
    return new_x, new_x_mi, new_time, new_label, new_diags, new_mask1, new_mask2, new_mask3
